...
export default async function CompanyDetailPage({
  ...
  const isOwnCompany = session?.user?.id === company.ownerId;
  const contactAccess =
    session?.user && !isOwnCompany ? await canContact(session.user) : null;
    ...
    <main className="mx-auto max-w-2xl px-4 py-10">
      ...
      <div className="mt-8 flex flex-col gap-6">
        ...
        <section className="flex flex-wrap gap-3">
          ...
          {contactAccess && <ContactButton targetUserId={company.ownerId} access={contactAccess} />}
        </section>
      </div>
    </main>
  );
